...
/*   Updated: 2024/12/24 21:48:25 by welow            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (data->map.map_layout[i])
	{
		...
		while (data->map.map_layout[i][j])
		{
			if (data->map.map_layout[i][j] == '1')
				draw_square(data, j * MAP_PIXEL, i * MAP_PIXEL, MINIMAP_WALL);
			else if (data->map.map_layout[i][j] == '0')
				draw_square(data, j * MAP_PIXEL, i * MAP_PIXEL, MINIMAP_FLOOR);
			else if (((int)data->player.pos_x == j && (int)data->player.pos_y == i)
				|| data->map.map_layout[i][j] == 'P')
				draw_square(data, j * MAP_PIXEL, i * MAP_PIXEL, MINIMAP_PLAYER);
			j++;
		}
		i++;
	}
...
{
	data->img.img = mlx_new_image(data->mlx, data->map.map_width * MAP_PIXEL, data->map.maply_height * MAP_PIXEL);
	data->img.img_addr = (int *)mlx_get_data_addr(data->img.img, &data->img.pixel_bits,
