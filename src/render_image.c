...
/*   Updated: 2024/12/08 21:50:01 by welow            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*add_minimap_line(t_data *data, int y)
{
	...
	while (x < data->minimap.size && x < data->map_width)
	{
		...
		else if (data->map[y + data->minimap.offset_y][x + data->minimap.offset_x] == '1')
			line[x] = '1';
		else if (data->map[y + data->minimap.offset_y][x + data->minimap.offset_x] == '0')
			line[x] = '0';
		else
			line[x] = '\0';
		x++;
...
void	render_minimap(t_data *data)
{
	...
	img_size = MAP_PIXEL + data->minimap.texture_size;
...
void	put_minimap(t_data *data)
{
	...
	data->minimap.offset_x = get_map_offset(data, data->map_width, (int)data->player.pos_x);
	data->minimap.offset_y = get_map_offset(data, data->map_height, (int)data->player.pos_y);
